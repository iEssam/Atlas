...
- [ ] Catalog-signed signature detection (`WTD_CHOICE_CATALOG`/`CryptCATAdmin`) + signing-cert subject as publisher — deferred; today catalog-signed system binaries (e.g. powershell.exe) show as "Unsigned" under file-based `WinVerifyTrust`, and publisher comes from the version resource
- [ ] Rules engine + profiles + simulation · boot analysis · scheduled tasks · network inspector · battery/thermal · experiments · MCP server · advanced privacy alerts (subsequent rounds)
