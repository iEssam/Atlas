...
- [ ] Extended retention tiers (TSDB T1/T2) · expert security metadata · signed plugin framework · remote support bundle · kernel-driver decision gate (subsequent rounds)
...
- **2026-07-14 — `cargo clippy` now blocked by the machine App Control policy** (`cargo-clippy.exe`, os error 4551) — it ran fine earlier in the session, so the policy tightened or clippy's binary was rebuilt and re-flagged. Substitute gate when blocked: `cargo build --workspace --all-targets` (rustc warnings) + `cargo fmt --check` + `cargo test --workspace`. Clippy needs an elevated/policy-exempt run (or CI) to re-verify. Same policy also blocks freshly-built service/UI exes intermittently — see the App Control notes above.
